...
const HRDashboard = () => {
  ...
  // Sort employees by hire date based on sortOrder
  const sortedEmployees = filteredEmployees.sort((a, b) => {
    const hireDateA = new Date(a.hire_date);
    const hireDateB = new Date(b.hire_date);

    return sortOrder === "asc" ? hireDateA - hireDateB : hireDateB - hireDateA;
  });
  ...
  // Handle sorting when the hire date column header is clicked
  const handleSort = () => {
    ...
    <div className="container mt-5">
            ...
            {currentEmployees.length > 0 ? (
              currentEmployees.map((emp, index) => (
                <tr key={emp.id}>
                  ...
                  <td>{emp.status}</td>
                  <td>
                    <Button variant="info" size="sm" onClick={() => openModal(emp)} className="me-2">
                      Details
                    </Button>
                    <Button
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered>
        ...
        <Modal.Body>
          {selectedEmployee && (
            <>
              ...
              <p><strong>Location:</strong> {selectedEmployee.location || "N/A"}</p>
        ...
        <Modal.Footer>
          ...
          <Button variant="warning" onClick={() => handleEdit(selectedEmployee.id)}>
            Edit
          </Button>
